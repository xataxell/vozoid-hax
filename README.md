...
‭
> [!TIP]
> Certain elements such as keybinds, colorpickers, sliders and more can be connected to other elements such as toggles, example below:
```lua
...
local scrollabledropdown = leftsection:Dropdown{
    ...
    ScrollingMax = 3, -- caps the amount of visible options before scrolling (if its 1 then you can only see one option at a time, if its 2 then you can only see two options at a time, and so on..)
